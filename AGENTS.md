...
**การ์ดไฟล์แบบไทล์** — ไอคอนไฟล์ + ชื่อ + วันที่ + ขนาด เรียงต่อจากปุ่มอัปโหลด
ใช้ `variant="plain"` (ไอคอนลอย ไม่มีกล่องครอบ) คู่กับ `AttachmentFileIcon`
ซึ่งวาดป้ายนามสกุลไฟล์ให้เองจากชื่อไฟล์

```tsx
<div className="flex flex-wrap items-start gap-3">
  <FileUpload variant="tile" accept=".pdf,.png,.jpg" multiple onFilesAccepted={add}>
    <FileUploadIcon><FileUpIcon /></FileUploadIcon>
    <FileUploadLabel>อัปโหลดไฟล์</FileUploadLabel>
  </FileUpload>
  ...
  <Attachment orientation="vertical" size="sm"
    ...
    <AttachmentMedia variant="plain">
      <AttachmentFileIcon name={file.name} />   {/* ได้ป้าย PDF / PNG / JPG เอง */}
    </AttachmentMedia>
    <AttachmentContent className="w-full items-center gap-0">
      <AttachmentTitle className="w-full text-xs">เอกสารเคลม</AttachmentTitle>
      <AttachmentDescription className="w-full">15/09/2025</AttachmentDescription>
      <AttachmentDescription className="w-full">6.6MB</AttachmentDescription>
    </AttachmentContent>
    <AttachmentActions className="absolute -top-2 -right-2">
      <AttachmentAction aria-label="เอาออก" variant="secondary"
        className="rounded-full border border-border shadow-sm" onClick={remove}>
        <XIcon />
      </AttachmentAction>
    </AttachmentActions>
  </Attachment>
...
- `items-start` ที่แถวสำคัญ — ไม่ใส่แล้วไทล์อัปโหลดจะยืดสูงตามการ์ดไฟล์ที่มีสามบรรทัด
- `AttachmentFileIcon` รับ `name` (ตัดนามสกุลเอง) หรือ `ext` ตรง ๆ ก็ได้
  ยาวเกิน 4 ตัวอักษรจะย่อฟอนต์ให้แทนการดันป้ายยาวออก ไอคอนทุกใบจึงกว้างเท่ากัน
- จำกัดจำนวนไฟล์ให้ซ่อน `FileUpload` ทิ้งเมื่อครบ ไม่ใช่ปล่อยให้กดแล้วเด้ง error

| prop ของ `AttachmentMedia` | ค่า |
|---|---|
| `variant` | `icon` (ไอคอนในกล่องพื้นเทา) · `image` · `video` (มีปุ่มเล่นกลางรูป) · `plain` (ไอคอนลอย ไม่มีกล่อง) |
...
- **`state="error"` ที่ยังไม่มีรูป/วิดีโอจริง (ไม่ใส่ลูกข้างใน) จะขึ้นไอคอนรูปกากบาท
  โทน `--danger` แทนไอคอนรูปธรรมดาโทนเทา** — ตั้งใจให้หน้าตาต่างจาก "ยังไม่มีรูป"
  เห็นชัดว่านี่คือ "พัง" ไม่ใช่แค่ยังไม่มีข้อมูล ถ้ามี `<img>` จริงอยู่แล้วจะไม่ทาสีทับ
  (`variant="icon"` ก็ทาโทนแดงทั้งกล่องตอน error เหมือนกัน · `variant="plain"`
  ไม่มีกล่องให้ทา จึงทาแค่สีไอคอนเป็น `text-destructive`)
...
- `variant` มี `dropzone` (ค่าเริ่มต้น กล่องขอบประเต็มความกว้าง) และ `tile`
  (กล่องย่อส่วนกว้าง 160px สูงตามเนื้อหา วางเป็นช่องแรกของแถวการ์ดไฟล์)
  ไทล์ใช้ขอบประสีแบรนด์ + พื้นอ่อน `--brand` เหมือน dropzone แค่ย่อขนาดลงมา —
  เดิมเคยใช้พื้นทึบไม่มีขอบเพราะกลัวขอบประไปตีกับขอบทึบของการ์ดไฟล์ข้าง ๆ
  แต่ของจริงขอบประสีแบรนด์กับขอบทึบสีเทาคนละโทนกันชัดอยู่แล้ว ไม่ปนกัน
  `FileUploadIcon` / `Label` / `Hint` ปรับขนาดตาม variant ให้เอง ไม่ต้องสั่งทับ
- ไทล์ที่มีทั้งไอคอน + label + ปุ่ม ใส่ปุ่มเป็น `<span aria-hidden>` แต่งด้วย
  `buttonVariants(...)` ไม่ใช่ `<button>` จริง เพราะกดที่ไหนของกล่องก็เปิด
  ตัวเลือกไฟล์อยู่แล้ว (ทั้งกล่องมี `role="button"`) ใส่ `<button>` ซ้อนจะผิด HTML
  (ปุ่มในปุ่ม) ดูตัวอย่างที่ `/components` หมวด **ไฟล์แนบ & รูปภาพ**
...
- **ไฟล์ที่ต้องสั่งพิมพ์/ขีดมาร์กใช้ตัวนี้ (เลื่อนดูหลายไฟล์ได้ด้วย) · รูปล้วน ๆ
  ที่แค่อยากเลื่อนดูเบา ๆ ไม่ต้องพิมพ์/ขีดมาร์กใช้ `MediaViewer`**
- วางตัวเป็น **หน้าเอกสาร** ไม่ใช่กล่องเด้ง — แถวบนเป็น **ลูกศรย้อนกลับ** (ไม่ใช่กากบาท)
  กับชื่อไฟล์ แถวถัดมาเป็นแถบเครื่องมือ และไม่มีแผงแก้เอกสารด้านข้าง
- **เลื่อนดูหลายไฟล์ได้ด้วย `onPrevious` `onNext`** — ปุ่มลูกศรวงกลมลอยซ้าย/ขวา
  กลางกรอบเอกสาร (หน้าตาแบบเดียวกับ `MediaViewer`) โผล่เฉพาะตอนส่ง prop มา
  ไม่ส่งมาเลย = ดูไฟล์เดียว ไม่มีปุ่ม **component ไม่ถือรายการไฟล์เอง** ฝั่งที่ใช้
  เป็นคนสลับ `file` prop เองตอนกด (แบบเดียวกับ `onFilesAccepted` ของ `FileUpload`)
  `hasPrevious` `hasNext` คุมปุ่มหรี่เองตอนชนขอบ (ไม่ส่งมาถือว่าเลื่อนได้เสมอ)
  `counter` ใส่ข้อความนับ เช่น `"2 / 5"` ไปโชว์ต่อท้าย meta ใต้ชื่อไฟล์
  **ลูกศรซ้าย/ขวาบนคีย์บอร์ดใช้ได้ด้วย** ไม่ต้องเอื้อมไปกดปุ่ม — คนละเรื่องกับ
  ปุ่มเลื่อน**หน้า**ของ pdf ในแถบเครื่องมือ (อันนั้นเลื่อนหน้าในไฟล์เดียวกัน
  อันนี้เลื่อนข้ามทั้งไฟล์)
...
- **pdf หลายหน้าต่อกันเป็นแถวยาว เลื่อนดูได้ตรง ๆ** ปุ่มเลื่อนหน้ากับรูปย่อเป็นแค่
  ทางลัด ไม่ใช่ทางเดียวที่จะข้ามหน้า · เลขหน้าบนแถบเครื่องมือขยับตามที่เลื่อนเอง
  (วัดจากหน้าที่กินพื้นที่ในจอมากที่สุดด้วย IntersectionObserver)
  หน้าที่เลื่อนพ้นจอไปแล้วจะคืน canvas ทิ้ง ไม่งั้นเอกสารร้อยหน้ากินหน่วยความจำเป็นร้อยเมกะไบต์
  มีปุ่มหุบแถบรูปย่อในแถบเครื่องมือสำหรับจอแคบ · ปิดถาวรด้วย `showThumbnails={false}`
  **แถบนี้กว้างตายตัว 128px จึงพับเก็บไว้เป็นค่าเริ่มต้นเองบนจอแคบกว่า `sm`
  (มือถือ)** เช็คแค่ตอนเปิดหน้าต่างครั้งแรก ไม่ตามหน้าจอหมุนหรือ resize — ไม่งั้น
  128px กินพื้นที่เกิน 1 ใน 3 ของ dialog จนตัวเอกสารเหลือที่อ่านไม่ออก ผู้ใช้กด
  ปุ่มเปิดเองได้ตามปกติถ้าต้องการ (เดสก์ท็อปยังเปิดเป็นค่าเริ่มต้นเหมือนเดิม)
- **ซูม 50–300%** ใช้ได้ทั้ง pdf และรูป — ปุ่ม `−` `+` · กดที่ตัวเลข % เพื่อกลับ 100% ·
  **จีบนิ้วบนแทร็กแพดหรือ Ctrl + ล้อเมาส์** บนตัวเอกสาร
  รูปซูมด้วยความกว้างจริงไม่ใช่ `transform: scale` พื้นที่เลื่อนจะได้โตตาม
  ส่วน pdf เรนเดอร์ใหม่ที่ scale นั้นเลย ซูมแล้วจึงคมไม่แตก
- **ขีดมาร์กได้ทั้ง pdf และรูป** — ปากกา/ยางลบ · สี 5 สี · หัวปากกา 3 ขนาด · เลิกทำ
  ปิดด้วย `showMarkup={false}` · **รอยขีดแยกตามหน้า** ขีดหน้า 1 ไม่ไปโผล่ทับหน้า 2
  **ยางลบต้องลากทับเส้นถึงจะลบ** ไม่ใช่กดทีเดียวหายทั้งหน้า (กู้คืนยากเกินไป)
  เลือกสีหรือขนาดเมื่อไหร่จะสลับกลับเป็นปากกาให้เอง
  **สีปากกาอ่านจาก token ตอนวาด** (canvas รับได้แต่ค่าสีจริง) จึงเปลี่ยนตามแบรนด์เอง
  ห้ามเปลี่ยนไปฝังค่าสีตรง ๆ
- **มีรอยขีดอยู่ = ปุ่มดาวน์โหลดให้ไฟล์ที่รวมรอยขีดแล้ว** (PNG ของหน้าที่เห็นอยู่)
  ไม่มีปุ่ม "บันทึกสำเนา" แยกอีกปุ่ม · ไม่มีรอยขีดก็ได้ไฟล์ต้นฉบับตามปกติ
  สั่งพิมพ์ก็เหมือนกัน มีรอยขีดจะได้ฉบับที่รวมรอยขีดไปด้วย
- **ปิดหน้าต่างแล้วรอยขีด/ซูม/หน้าที่เปิดค้างไว้ล้างทิ้งหมด** รอยขีดเป็นของชั่วคราว
  จนกว่าจะดาวน์โหลดหรือพิมพ์ออกไป ไม่มีการเก็บกลับเข้าไฟล์ต้นทาง
...
> **pdf เรนเดอร์เองด้วย pdf.js ไม่ได้ฝัง `<iframe>` ให้เบราว์เซอร์เปิด** — ตัวอ่าน pdf
> ของเบราว์เซอร์เป็นหน้าต่างของมันเอง สไตล์ DS ไปไม่ถึงสักจุดและขีดทับไม่ได้
> พอวาดลง canvas เองแล้วทุกอย่างในหน้าต่างเป็นของเราหมด
>
> - `pdfjs-dist` โหลดแบบ **dynamic import** ไม่ติดไปกับ bundle แรกของหน้า
> - worker สร้างจาก `new Worker(new URL("pdfjs-dist/build/pdf.worker.min.mjs", import.meta.url))`
>   **ห้ามเปลี่ยนไปชี้ CDN** DS ต้องทำงานหลังกำแพง CSP ได้และไม่ยิงเน็ตออกนอก
> - เปิดไม่สำเร็จ (worker โหลดไม่ผ่านในแอปปลายทางบางตัว) จะตกไปใช้ `<iframe>`
>   ของเบราว์เซอร์ให้เอง ไม่ปล่อยจอว่าง
> - รูปย่อทำให้ไม่เกิน 40 หน้า และไล่เรนเดอร์ทีละใบ เพราะ pdf.js มี worker เส้นเดียว
>   ยิงพร้อมกันทั้งเล่มจะแย่งคิวกันจนหน้าหลักมาช้า
