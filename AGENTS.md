...
| แกน | ค่า | ผลที่ได้ |
|---|---|---|
| `data-tint` | `pure` · `soft` · `deep` | สีกลางผสมสีแบรนด์ ไม่ผสม · บาง · ชัด |
...
- `soft` / `deep` ใช้ `color-mix()` กับ `var(--primary)` ซึ่งคลี่เป็นสีของแบรนด์ที่ใช้อยู่
  **กฎชุดเดียวจึงครอบคลุมทุกแบรนด์ เพิ่มแบรนด์ใหม่ไม่ต้องมาแก้ `tint.css`**

ทั้งสามระดับใช้สเกลฐานเดียวกัน จึงเรียงจากผสมน้อยไปมากจริงทุกแบรนด์ทุกโหมด
(วัดค่าต่าง R-G-B ของสีพื้น: `pure` 0 · `soft` 5-10 · `deep` 10-25)

**ถ้าไม่ใส่ `data-tint` เลยจะได้ค่าที่ไฟล์แบรนด์เขียนไว้เอง ซึ่งไม่ได้อยู่บนสเกลนี้**
ค่าพวกนั้นจูนด้วยมือตอนสร้างแต่ละแบรนด์ ไม่ได้อิงกฎร่วมกัน จึงคาดเดาไม่ได้ —
siam โหมดมืดผสม 19 (มากกว่า `soft`) แต่ parich โหมดมืดผสมแค่ 6 (น้อยกว่า `soft`)
