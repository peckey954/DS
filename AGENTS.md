...
`brand` และ `destructive` ใช้สีจาง **ทับพื้นหลังเดิม** ไม่ใช่สีทึบ จึงกลับด้าน
เองในโหมดมืด และ `brand` ก็เปลี่ยนตามแบรนด์ให้อัตโนมัติเพราะผูกกับ `--primary`

**`brand` ต้องใช้คลาสชุดเดียวกับสถานะ "ถูกเลือก" ของกรอบ radio/checkbox**
(`FieldLabel` ใน `field.tsx`) เป๊ะ ๆ คือ `border-primary bg-primary/5 dark:bg-primary/10`
เพื่อให้ผู้ใช้เห็นว่า "สีนี้ = สีของแบรนด์" เป็นสีเดียวกันทั้งระบบ — **ถ้าแก้ที่ใดที่หนึ่ง
ต้องแก้อีกที่ด้วย**

**`warning` ไม่ได้ใช้สีจาง** แต่ใช้ token พื้นทึบของตัวเอง เพราะสีเหลืองจางทับพื้นขาว
ให้ผลออกเบจอมน้ำตาล ไม่ใช่เหลืองสด ค่าจึงตั้งให้ตรงกับ `amber-50` / `amber-900` /
`amber-200` ของ Tailwind เป๊ะ (วัดแล้วได้ `rgb(255 251 235)` ตรงกับ `amber-50` พอดี)
...
| token | ใช้ตอนไหน | ค่าโหมดสว่าง |
|---|---|---|
| `--warning` | **พื้นทึบ** ของ warning (ทำงานเหมือน `--card`) | `amber-50` |
| `--warning-foreground` | ตัวอักษรบนพื้นนั้น | `amber-900` |
| `--warning-border` | เส้นขอบ | `amber-200` |
| `--destructive-subtle` | ตัวอักษรบนพื้นจางของ destructive | — |

โหมดมืดสลับหัวท้าย พื้นเป็น `amber-950` ตัวอักษรเป็น `amber-50` ขอบเป็น `amber-900`
