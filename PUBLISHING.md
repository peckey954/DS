...
| แพ็กเกจ | เนื้อหา |
|---|---|
| `@peckey954/ui` | component ทั้ง 60 ตัว + `cn()` + คำศัพท์กลาง (`globals.css`) |
...
ตัวที่ใช้อยู่ชื่อ `publish-ds` เป็น **Granular Access Token หมดอายุ 10 ส.ค. 2569**
(npm ส่งเมลเตือนล่วงหน้า 1 วัน) — npm ไม่ให้ตั้งแบบ "ไม่หมดอายุ" อีกแล้ว
ตอนสร้างใหม่เลือกได้สูงสุด 90 วัน **สร้างใหม่แล้วมาแก้บรรทัดนี้ด้วยทุกครั้ง**

**token หมดอายุไม่ได้ทำให้แพ็กเกจหาย** ของที่ปล่อยไปแล้วอยู่บน npm ตลอดไป
คนอื่นยัง `pnpm add` ได้ตามปกติ สิ่งเดียวที่ทำไม่ได้คือปล่อยเวอร์ชันใหม่จากเครื่อง

> **ถ้าไม่สร้างใหม่ ให้ลบบรรทัด `//registry.npmjs.org/:_authToken=…` ออกจาก `~/.npmrc` ด้วย**
> npm แนบ token ที่ตายแล้วไปกับทุกคำขอ บางทีเจอ `E401` ตั้งแต่ตอน `pnpm install`
> ทั้งที่แค่จะดึงแพ็กเกจสาธารณะ ลบทิ้งแล้วมันจะกลับไปเป็นผู้ใช้ทั่วไปที่อ่านได้ปกติ
...
> **ยังไม่เคยปล่อยจริงด้วยทางนี้เลย** — เช็คได้จาก `npm view @peckey954/ui@0.2.1 --json`
> แล้วดู `dist.attestations` ถ้าเป็น `null` แปลว่าเวอร์ชันนั้นปล่อยจากเครื่อง
> (ปล่อยผ่าน OIDC จะมี provenance ติดมาเสมอ) เวอร์ชันหน้าลองทางนี้ก่อน
> ถ้าพังก็ยังปล่อยจากเครื่องได้เหมือนเดิม ไม่ได้ปิดทางเก่า
